...
class PandasLoop(MicroWriter):
    ...
    # One column per stream. Not to many columns!

    def __init__(self, df, interval, write_key, origin, with_copulas=False):
        """
        :param df:            pd.DataFrame   (row names become stream names, though need not have .json)
        :param interval:      float          Minutes between data points
        :param write_key:     str            A MUID self.new_key
        :param origin:        float          Epoch time corresponding to first row
        """
